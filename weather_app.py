...
class WeatherCrawler(QThread): # 쓰레드 클래스로 선언
    ...
    def weather_crawling(self, weather_area):
        ...
        try:
            ...
            today_temper = weather_soup.find('div', {'class': 'temperature_text'}).text  # 오늘기온 크롤링
            today_temper = today_temper[6:9].strip()  # 오늘 기온만 인덱싱한(추출) 후 양쪽 공백문자 제거
            yesterday_weather = weather_soup.find('p', {'class': 'summary'}).text  # 어제 날씨비교 크롤링
            yesterday_weather = yesterday_weather[0:13].strip()
            today_weather = weather_soup.find('span',
                                              {'class': 'weather before_slash'}).text  # 오늘 날씨(ex:맑음, 흐림, 구름 많음...)
            ...
            try:
                area_title = weather_soup.find('span', {'class': 'btn_select'}).text  # 날씨를 검색한 지역명 크로링
                area_title = area_title.strip()
                today_temper = weather_soup.find('span', {'class': 'todaytempe'}).text  # 오늘기온 크롤링
                today_temper = f"{today_temper}°"
                today_weather = weather_soup.find('p', {'class': 'cast_txt'}).text  # 오늘 날씨(ex:맑음, 흐림, 구름 많음...)
...
class WeatherApp(QMainWindow, form_class):
    def __init__(self, parent=None):  # 초기화자
        ...
        self.statusBar().showMessage('Weather Applicatioin Ver 1.0')  # 윈도우 상태표시줄 입력
        self.weatherInfo = WeatherCrawler(self) # 날씨 크롤러클래스의 객체를 생성
    ...
    def weather_start(self):
        ...
        if input_area == '':
            ...
            # weather_crawling 함수의 리턴값 저장(크롤링한 날씨 정보 리스트 저장)
            ...
            if weather_data[7] == 0:
                QMessageBox.about(self, '지역명오류!', '입력하신 지역명을 다시 확인하시기 바랍니다다.')
